...
public class EventLogger implements DemoPlayer.Listener, DemoPlayer.InfoListener,
    ...
    //状态改变 播放时长，准备就绪就播放，播放状态（缓冲B，播放结束E，准备就绪R，正在准备P，空闲I）
    @Override
    ...
    @Override
    public void onBandwidthSample(int elapsedMs, long bytes, long bitrateEstimate) {//经过时长，字节，估算比特率
        Log.d(TAG, "bandwidth [" + getSessionTimeString() + ", " + bytes + ", " + getTimeString(elapsedMs) + ", " + bitrateEstimate + "]");
    }

    @Override
    public void onDroppedFrames(int count, long elapsed) {//丢帧数
